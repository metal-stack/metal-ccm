	...
	_, controller := cache.NewInformer(
		...
		cache.ResourceEventHandlerFuncs{
			...
			UpdateFunc: func(oldObj interface{}, newObj interface{}) {
				...
				if err != nil {
					klog.Warning("newNode does not have a tunnelAddress, ignoring")
					return
				}
				if oldTunnelAddress == newTunnelAddress {
					klog.Info("node was not modified and calico tunnel address has not changed, not updating metallb config")
					return
				}

				klog.Info("node was modified and calico tunnel address has changed, updating metallb config")
